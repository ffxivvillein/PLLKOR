...
|작성자|작성일|👍|내용|
|-|-|-|-|
|Lumen@펜리르|2025.08.25 16:27|10|혹시 로그창을 하나에서 두 개 더 늘려주실 수 있나요?그리고 가능하면 단축바도 두 개 쯤 더 늘어난다면 좋겠습니다.|
|마영도@모그리|2025.08.25 21:03|9|단축바는 더 추가될 예정이 없나요? 직업도 많아지고 레벨도 높아지면 스킬도 많아지는데 단축바 10개는 너무 적은 것 같습니다........|
|경토@카벙클|2025.08.26 19:10|8|컨텐츠가 추가될수록 획득 가능한 의상은 많은데 투영 세트 및 옷장 갯수는 한정적이어서 슬퍼요 (ㅠㅠ) 남성 캐릭터와 여성 캐릭터를 왔다갔다하면 투영 세트도 일일이 고치느라 시간이 꽤 할애됩니다. 혹시 투영 세트를 남성용/여성용으로 늘리거나 저장 기능을 추가하실 생각은 없으신지, 옷장 갯수를 800개에서 늘려주실 수 없으실지 문의드립니다~. |
|체인칠링@카벙클|2025.08.25 17:34|8|안녕하세요. 멘토로 지내다보면 가끔 드물지만 특정 공포증을 지니신 분들이 초보자 채널에서 도움을 요청하는 경우가 있습니다. 그 중에서도 자주 보이는게 심해 공포증 분들인데요. 게임내에서는 아예 이분들을 돕기 위한 동아리도 있는걸로 알고 있습니다. 아무래도 특정 확장팩에서는 물속에 들어가야하기도하니까요. 혹시 이런 분들 용으로 따로 필터를 제작해주실 생각이 없을까요?|
|로츠@초코보|2025.08.26 19:12|7|모그레터를 한번에 삭제할수있게 해주실수있나요?|
|야슈헤약@톤베리|2025.08.25 19:15|7|보스의 대사 또는 전투의 대사 (황금오면서 화면 중앙의 대사알림창 뜨는거)자체를 옮길순 없나요?이게 hud를 수정하는데에 이 대사창이 화면을 가리거나.. hud를 가리는둥 여러가지로 불편함이 있습니다..|
|초담@펜리르|2025.08.25 18:53|7|단축바 갯수를 늘릴 계획이 있으신지 궁금합니다~ 더 있으면 좋겠다라는 생각이 계속 들어서요몇개 더 생겼으면 좋겠어요|
|엘른@초코보|2025.08.26 23:41|6|요즘 전투 컨텐츠를 할 때 너무 광원이 심합니다 .. ㅠㅠ...이제 유저들의 평균 나이도 늘어가고 있는데, 광원을 줄이는 옵션을 내주시거나 연출적으로 광원을 아예 줄여주실 생각은 없으실까요...? 물론 모니터의 빛을 낮추면 되는 일이지만 이러면 또 이제 화면이 보이질 않아서 게임을 하는데 너무 불편합니다. 모쪼록 광원을 조금 줄여주시면 좋을 거 같습니다.사유 : 광원이 너무 강해서 가끔 기믹도 안보이고 눈이 시려요...광원 예시 : 페이즈 전환이나 보스가 기믹을 할 때 모니터 전부 흰 빛으로 가득차거나, 절 오메가 토벌전 1페 탱버 등등..|
...
|반짝반짝작은별@카벙클|2025.08.25 21:04|6|안녕하세요 HUD UI 사이즈 크기를 추가해 주실 수 있을지 궁금합니다. 60%는 너무 작고 80%는 좀 크게 느껴져서 70% 같은 게 추가되었으면 합니다.|
|할라할라@모그리|2025.08.25 16:22|6|포기투표/추방투표가 올라올때 누가 투표를 올렸는지 표시해주세요! 조율되지않은 투표가 올라오는게 보기 힘듭니다|
|부장판사@카벙클|2025.08.25 16:16|6|안녕하세요. 평소 색각 보정 기능을 이런저런 이유로 이용하고 있는 유저입니다. 다름이 아니라, 배틀 컨텐츠에서 사용되는 플레이어의 스킬 이펙트나 보스의 기술 이펙트를 색각 보정 기능처럼 자체적으로 조절할 수 있는 시스템은 구현이 힘들까요? 꽤 예전부터 비슷한 질문이 들어갔던 것으로 보이는데, 고레벨 스킬임을 강조하기 위해서 이펙트 자체도 화려해질 수밖에 없다면 적어도 옵션에서 광량을 조절할 수 있었으면 좋겠습니다. 슬슬 눈이 아파요...|
|오터씨@카벙클|2025.08.25 16:55|5|부대제작을 할 때 제발 재료를 한번에 넣게 해주실수 있을까요 한 재료당 3번씩 넣는데 손목이 너무 아픕니다 ㅠㅠ |
...
|작성자|작성일|👍|내용|
|-|-|-|-|
|쟤로@톤베리|2025.08.31 23:57|0|하드컨텐츠를 하다 보면 상태바를 계속 확인해가며 플레이해야 하는데요, 효월 이후부터는 색깔로 구분해야만 하는 디버프나 기믹이 많이 줄어들고 차이가 쉽게 보이게끔 바뀌는 것 같습니다.(이온 디버프 등)이런 부분을 디자인할 때 특별히 고려하는 점이라거나 중요하게 여기는 부분이 있을지 궁금합니다|
|윤하란@모그리|2025.08.31 21:55|0|아우라 일부 얼굴유형의 뿔을 보면 뿔 끝부분은 색깔이 옅거나 짙은데요! 이 부분은 아직 덜 자란 것인지, 아니면 자란지 오래되어 변색된 것인지 궁금합니다.|
|Dil@톤베리|2025.08.31 19:29|0|안녕하세요, 혹시 서버초월 링크쉘의 개수를 늘려주실 수 있을까요? 이전에 동일 서버에 있었던 친구들도 시일이 지나며 다른 서버로 이전하는 경우도 있었고, 다양한 서버의 사람들과 소통하는 것이 가능해지면서 본인의 서버 뿐만 아니라 다른 서버의 사람들과 인연을 쌓을 수 있게 되었습니다. 때문에 이런 저런 이유로 링크쉘보다 서버 초월 링크쉘을 더 많이 사용하게 되었는데,  서버 초월 링크쉘의 개수는 8개로 한정되어있는 탓에, 기존에 함께 이용하고 있던 링크쉘을 삭제해야하는 경우가 생겨 아쉬운 마음에 문의 드립니다.|
|아에키@펜리르|2025.08.31 16:45|0|안녕하세요! 저는 단체 자세로 파판14의 스크린샷을 많이 찍고 있는 모험가입니다.다른 유저의 하우징에서 스크린샷을 찍는 걸 좋아하는데 하우징의 분위기를 맞춰 색상 필터를 고르다 보니 파판14의 단체 자세 색상 필터는 많은데 주로 쓰는 색상 필터는 환하게 1~4만 쓰게 되더라고요(그거마저 환하게 2 필터는 안 쓰고 있습니다.) 혹시 단체 사진의 색상 필터를 추가할 계획은 있으신가요?|
|chacha***|2025.08.31 16:36|0|단체 자세에서 오다기리 케이님이 추천하시는 카메라 셋팅이 있으신가요?!개발자가 즐겨쓰는 카메라&조명 셋팅이 궁금합니다!!|
|이카데@초코보|2025.08.31 15:57|0|하우징 가구에 2염색을 도입하실 생각이 있으신가요?|
|실베스타@모그리|2025.08.31 13:59|0|편의성 패치가 아직 많이 부족한거 같은데 추가 의향이 있는지 궁금합니다1. 확장팩마다 추가된 컨텐츠 한번에 볼 수 있는 UI 추가 (확장팩마다 컨텐츠는 많아지고 지역도 많아지는데 가이드는 필요하다고 생각합니다.)2. 전투 고대 무기 및 채집 및 제작 고대 도구에 대한 UI 추가 (예시 : 시작 위치, 필요 재료 목록 등)3. 임무 정보 - 단골 거래 UI에서 단골 거래 의뢰인 이름옆에 위치 혹은 누르면 지도라도 떠서 텔레포 탈 수 있도록 기능 추가4. NPC 기능 식별 편하도록 추가 (예시: 조디악웨폰 주간임무, 아니마웨폰 주간임무) -> 지도에서 NPC위에 커서 올리면 바로 확인도 가능하게 기능 추가5. 임무 정보 개선 요청 -> 주제한 컨텐츠를 이곳에서 더 많이 볼 수 있으면 좋겠음 -> 유저가 원하는거만 보도록 커스텀 기능도 같이 있으면 좋을거 같음6. 마물 사냥 전리품에 전송망 이용권 추가 해주거나 마물 휘장 센추리온 휘장을 마물 사냥 전리품으로 통일하면 좋을거 같습니다. 전송망 이용권 구할려면 신생~홍련 마물 잡아야하는데 A급은 혼자서도 토벌이 가능해서 젠 되어있는 경우가 없어서 잡지를 못함7. 스킬 설명 개선 (예시 : A 스킬 발동 조건 : A 스킬 시전 가능 )8. 아이템 획득처 상세하게 추가 (아이템 설명에 간략하게라도 던전에서 획득, 제작으로 획득, 채집으로 획득 이런식으로 추가)9. 나무인형 토벌전 시작 위치 통일 혹은 나무인형 토벌전 NPC 위치 표시 기능 추가 (복귀 후 딜싸이클 연습겸 나무인형 토벌전 갈려고해도 NPC 위치 생각이 안남....)10. 지도에 교환원이라고만 나올게 아니라 무슨 무슨 교환원인지 볼 수 있게 패치 부탁드립니다. 11. NPC에 대한 정보 조금더 상세하게 패치 (예시 : 화폐 교환원 -> 채집 및 제작 화폐 교환원)|
|비디오게임@모그리|2025.08.31 12:09|0|초보자(새싹)의 조건(메인퀘스트and누적플레이타임)을 캐릭별이 아닌 계정으로 바꿔주세요.현재 시스템은 오히려 초보자를 도와주는데 혼동을 주고 악용될 여지도 있는 시스템이라 생각합니다.예를들어 계정 내 캐릭터 중에 메인퀘스트and플레이타임 조건을 만족한 캐릭터가 있다면 초보자마크를 없애주세요|
|라오나라@카벙클|2025.08.31 00:13|0|안녕하세요. 스크린샷을 많이 찍는 유저입니다.악기를 들고 연주하는 모습의 스샷을 좀 더 다양하게 찍고 싶은데 연주 스킬에 있는 악기 오브젝트가 악기들에비해 많이 없는 것이 아쉽습니다. 악기에 맞는 오브젝트들을 추가해주실 수 있는지 궁금합니다.|
|마르@펜리르|2025.08.30 22:49|0|전투장비, 채제작 장비, 고대무기가 인벤토리 무기 칸에 동시에 수납돼서 자리가 모자랍니다. 3개를 카테고리별로 분리하거나 최소한 전투장비, 채제작 장비 칸을 분리하면 좋을 것 같습니다.|
|마르@펜리르|2025.08.30 22:49|0|우편함 선택, 일괄수령 이 추가됐으면 좋겠습니다. 소지품도 하나씩 정리하기 번거로운데 여러 개를 동시에 선택해서 판매, 버리기, 옮길 수 있는 기능이 있으면 좋을 것 같습니다.|
|bb***|2025.08.30 21:47|0|한국 유저들은 보통 키보드와 마우스로 플레이하는 유저들이 많은데 일본에서는 게임패드를 이용하는 유저들이 많은 것으로 알고 있습니다. 파판에서는 두가지 조작 방식을 모두 지원하고 있고 이에 맞는 UI를 제공하고 있는데요두가지 UI를 동시에 개발하는 과정에서 어떤 어려움을 겪고 해결하셨는지 궁금합니다.|
|부리또냠냠이@초코보|2025.08.30 21:23|0|장면 연출, 보스 스킬 이펙트의 광량이 줄었으면 좋겠습니다. 어렵다면 최소한 이펙트 간략화 혹은 on/off 기능이 추가되길 희망합니다.인게임에서 강한 빛을 활용한 효과가 갈수록 늘어남에 따라 눈의 피로도 또한 가중되고 있습니다. 제 주변 유저 중 상당수는 강한 이펙트에 의해 단시간에 피로도가 급증하거나 눈이 아파 게임을 중단한 경험이 있습니다. 안구 관련 지병이 있는 친구는 게임 내의 강한 빛에 눈이 상할 위험이 있어 선글라스를 착용하고 플레이합니다. 장판이나 징, 보스 모션 같은 기믹 표시를 가리는 등의 부차적인 불편함도 있습니다.빛은 극적인 효과를 만들기에 훌륭한 도구이지만, 남발하면 게임 플레이를 방해하는 요소가 됩니다. 광량을 조절하는 적절한 패치를 통해 오래, 건강하게, 즐겁게 파이널판타지14를 플레이할 수 있길 바랍니다.|
...
|천사화@모그리|2025.08.28 16:42|2|많은 직업군과 더불어 그직업이 성장함에 따라 장비가 수시로 바뀌어 장비관리가 어려워집니다. 여러 직업군을 동시에 육성해야 하는경우에 장비의 관리가 어려워 장비함의 총량을 늘려 주시거나, 장비함에서 장비를 이동하게 되면 경고멘트나 인지가 가능한 시스템을 도입해 주시길 부탁드립니다.|
...
|고지현@카벙클|2025.08.26 21:10|2|인벤토리를 정리할 때 더는 쓰지 않는 아이템을 하나하나 버려야 하는 점이 다소 불편한데, 일괄적으로 버리거나 혹은 복수 선택하여 한 번에 버리는 기능을 구현할 수는 없나요?|
...
|Summergol@톤베리|2025.08.26 19:25|2|집사 소지품에 현재 소지품의 겹치는 아이템을 한번에 넣을 수 있는 것처럼 부대 금고에도 겹치는 아이템을 한번에 넣는 기능이 추가해주실 수 있나요?|
|로츠@초코보|2025.08.26 19:12|7|모그레터를 한번에 삭제할수있게 해주실수있나요?|
|경토@카벙클|2025.08.26 19:10|8|컨텐츠가 추가될수록 획득 가능한 의상은 많은데 투영 세트 및 옷장 갯수는 한정적이어서 슬퍼요 (ㅠㅠ) 남성 캐릭터와 여성 캐릭터를 왔다갔다하면 투영 세트도 일일이 고치느라 시간이 꽤 할애됩니다. 혹시 투영 세트를 남성용/여성용으로 늘리거나 저장 기능을 추가하실 생각은 없으신지, 옷장 갯수를 800개에서 늘려주실 수 없으실지 문의드립니다~. |
