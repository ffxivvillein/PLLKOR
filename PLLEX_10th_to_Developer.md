...
|실베스타@모그리|2025.08.31 13:59|0|편의성 패치가 아직 많이 부족한거 같은데 추가 의향이 있는지 궁금합니다1. 확장팩마다 추가된 컨텐츠 한번에 볼 수 있는 UI 추가 (확장팩마다 컨텐츠는 많아지고 지역도 많아지는데 가이드는 필요하다고 생각합니다.)2. 전투 고대 무기 및 채집 및 제작 고대 도구에 대한 UI 추가 (예시 : 시작 위치, 필요 재료 목록 등)3. NPC 기능 식별 편하도록 추가 (예시: 조디악웨폰 주간임무, 아니마웨폰 주간임무) -> 지도에서 NPC위에 커서 올리면 바로 확인도 가능하게 기능 추가4. 임무 정보 개선 요청 -> 주제한 컨텐츠를 이곳에서 더 많이 볼 수 있으면 좋겠음 -> 유저가 원하는거만 보도록 커스텀 기능도 같이 있으면 좋을거 같음5. 마물 사냥 전리품에 전송망 이용권 추가 해주거나 마물 휘장 센추리온 휘장을 마물 사냥 전리품으로 통일하면 좋을거 같습니다. 전송망 이용권 구할려면 신생~홍련 마물 잡아야하는데 A급은 혼자서도 토벌이 가능해서 젠 되어있는 경우가 없어서 잡지를 못함6. 스킬 설명 개선 (예시 : A 스킬 발동 조건 : A 스킬 시전 가능 )7. 아이템 획득처 상세하게 추가 (아이템 설명에 간략하게라도 던전에서 획득, 제작으로 획득, 채집으로 획득 이런식으로 추가)8. 나무인형 토벌전 시작 위치 통일 혹은 나무인형 토벌전 NPC 위치 표시 기능 추가 (복귀 후 딜싸이클 연습겸 나무인형 토벌전 갈려고해도 NPC 위치 생각이 안남....)9. 지도에 교환원이라고만 나올게 아니라 무슨 무슨 교환원인지 볼 수 있게 패치 부탁드립니다. 10. NPC에 대한 정보 조금더 상세하게 패치 (예시 : 화폐 교환원 -> 채집 및 제작 화폐 교환원)|
